...
function App() {
  ...
  const amplitudes = useMemo(() => (currentState ? enumerateAmplitudes(currentState, numQubits) : []), [currentState, numQubits])

  const maxColumns = useMemo(() => {
    const maxColumn = gates.reduce((acc, gate) => Math.max(acc, gate.column), 0)
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    const gateName = event.active?.data?.current?.gateName as string | undefined
    ...
    if (arity > numQubits) {
      setToast({ message: 'Increase the register size before placing that gate.', severity: 'error' })
      return
    }
    if (arity === 1) {
    ...
    setPendingPlacement({ gateName: gateName as PendingPlacement['gateName'], column: overData.column, primaryQubit: overData.qubit })
  }
  ...
  const handlePlacementConfirm = (targets: number[]) => {
    ...
    addGate(pendingPlacement.gateName as never, targets, pendingPlacement.column)
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      setToast({ message: 'Share URL copied to clipboard', severity: 'success' })
    } catch {
      setToast({ message: 'Copy failed. Manually copy from address bar.', severity: 'error' })
    }
  }
  ...
  const handleLoadFromStorage = () => {
    ...
    if (!stored) {
      setToast({ message: 'No saved circuit in this browser', severity: 'error' })
      return
    }
    loadCircuit(stored)
  ...
  const handleFileSelect = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setToast({ message: error instanceof Error ? error.message : 'Failed to import circuit', severity: 'error' })
    } finally {
      event.target.value = ''
    }
  }
  ...
  return (
    <Box sx={{ py: { xs: 4, md: 6 }, px: 2 }}>
      <Container maxWidth="xl">
        ...
        <Stack spacing={4} sx={{ mt: 4 }}>
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={4}>
            <Box sx={{ flex: 1, minWidth: 0, p: 3, borderRadius: 3, border: '1px solid rgba(132,150,255,0.25)' }}>
              <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
                ...
                <Button variant="contained" onClick={() => setMeasurementOpen(true)}>
                  Measure qubits
                </Button>
              </Stack>
              {measurement ? (
                <Box>
                  <Typography variant="subtitle2">Outcome: {measurement.outcome}</Typography>
                  {Object.entries(measurement.probabilities).map(([label, probability]) => (
      ...
      <input type="file" hidden ref={fileInputRef} accept="application/json" onChange={handleFileSelect} />
