...
export function decodeCircuitFromQuery(
  search: string,
...
function validateCircuitDefinition(
  input: CircuitDefinition,
): asserts input is CircuitDefinition {
  enforceQubitLimit(input.numQubits);
  if (input.initialState !== undefined) {
    if (!Array.isArray(input.initialState)) {
      throw new Error("Circuit initial state must be an array");
    }
    if (input.initialState.length !== input.numQubits) {
      throw new Error(
        "Circuit initial state length does not match qubit count",
      );
    }
    input.initialState.forEach((bit) => {
      if (bit !== 0 && bit !== 1) {
        throw new Error(
          "Circuit initial state must contain only 0 or 1 values",
        );
      }
    });
  }
  ...
  input.gates.forEach((gate) => {
    ...
    if (
      ...
      gate.targets.length !== definition.arity
    ) {
