...
import type { GateInstance, GateName, MeasurementResult, PackedMatrix, StateVector, TimelineEntry } from '@/types/quantum'
...
export function applyGateToState(state: StateVector, gateName: GateName, targets: number[], numQubits: number) {
...
export function buildTimeline(numQubits: number, gates: GateInstance[]): TimelineEntry[] {
  const orderedGates = [...gates].sort((a, b) => (a.column === b.column ? a.id.localeCompare(b.id) : a.column - b.column))
  const timeline: TimelineEntry[] = []
  let workingState = createZeroState(numQubits)
...
export function measureState(
  ...
  for (let basis = 0; basis < totalStates; basis += 1) {
    ...
    normalization += magnitudeSquared(collapsed.real[basis], collapsed.imag[basis])
  }
...
export function createGateInstance(name: GateName, targets: number[], column: number): GateInstance {
  return {
    id: nanoid(6),
    name,
    targets,
...
function matchesMask(basis: number, expectations: Map<number, number>): boolean {
...
function applySingleQubitGate(state: StateVector, matrix: PackedMatrix, targetQubit: number) {
...
function applyMultiQubitGate(state: StateVector, matrix: PackedMatrix, targets: number[], numQubits: number) {
  const { real, imag } = state
  ...
  for (let combo = 0; combo < combos; combo += 1) {
    ...
    for (let row = 0; row < blockSize; row += 1) {
      ...
      for (let col = 0; col < blockSize; col += 1) {
        ...
        const [prodReal, prodImag] = multiplyComplex(mReal, mImag, vectorReal[col], vectorImag[col])
        accReal += prodReal
        accImag += prodImag
      }
